...
import { 
  ...
  Settings2, Layers, Tag, Calculator, MoreVertical, Star, Loader2
} from "lucide-react";
...
export default function NovaComposicaoPage() {
  ...
  const createMutation = trpc.composition.create.useMutation({
    ...
    onError: (err) => {
      toast.error(`Erro ao criar composição: ${err.message}`);
    }
  });
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F3F4F6] overflow-hidden text-slate-700">
      ...
      <ItemSelectorDialog
        ...
        onAddNewInsumo={() => setIsInsumoDialogOpen(true)}
      />
      {/* Header Estilo ERP */}
      <header className="bg-white border-b px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-8">
           <div className="border-l-4 border-orange-400 pl-3">
             <span className="text-xs uppercase font-bold text-slate-400 block leading-none mb-1">Catálogo <Info className="h-3 w-3 inline" /></span>
           ...
           <div className="flex items-center gap-10">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-1 bg-slate-50 border px-2 py-0.5 rounded">
                  ...
                  <Settings2 className="h-3.5 w-3.5 text-slate-300 ml-1" />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="flex bg-slate-100 rounded border p-0.5">
                  <button 
                    ...
                    className={`text-[10px] font-bold px-3 py-1 rounded transition-all ${watch("isActive") ? 'bg-[#33b5e5] text-white' : 'text-slate-400'}`}
                  >ATIVO</button>
                  <button 
        ...
        <div className="flex items-center gap-2">
          <Button 
            ...
            className="bg-[#5cb85c] hover:bg-[#4cae4c] h-9 px-6 text-sm font-bold text-white shadow-sm flex items-center gap-2"
          >
            <Check className="h-4 w-4" /> Salvar
          </Button>
          <Button variant="outline" size="icon" className="h-9 w-9 text-slate-400 border-slate-200"><MoreVertical className="h-5 w-5" /></Button>
      ...
      <main className="flex-1 p-3 space-y-3 overflow-hidden flex flex-col">
        ...
        <div className="grid grid-cols-12 gap-3 flex-shrink-0">
          <div className="col-span-8 bg-white border border-slate-200 rounded p-4 grid grid-cols-12 gap-4">
             <div className="col-span-2 space-y-1">
                <Label className="text-xs font-bold text-slate-500 uppercase">Código: <span className="text-red-500">*</span></Label>
                <Input {...register("code")} className="h-9 border-slate-200 text-sm px-3" />
             </div>
             <div className="col-span-10 space-y-1">
                <Label className="text-xs font-bold text-slate-500 uppercase">Descrição: <span className="text-red-500">*</span></Label>
                <Input {...register("description")} className="h-9 border-slate-200 text-sm px-3 uppercase font-medium" />
             </div>
             
             <div className="col-span-3 space-y-1">
                <Label className="text-xs font-bold text-slate-500 uppercase">Unidade: <span className="text-red-500">*</span></Label>
                <Controller
                    name="unit"
                    control={control}
                    render={({ field }) => (
                      <UnitSelect value={field.value} onChange={field.onChange} placeholder="Selecione..." />
                    )}
                />
             </div>
             <div className="col-span-6 space-y-1">
                ...
                <Controller
                    ...
                    control={control}
                    render={({ field }) => (
                      <CompositionTypeSelect value={field.value} onChange={field.onChange} placeholder="Selecione o tipo..." />
                    )}
                />
             </div>
             <div className="col-span-3 space-y-1">
          ...
          <div className="col-span-4 bg-white border border-slate-200 rounded p-4 flex flex-col">
             ...
             <Textarea 
              ...
              className="flex-1 min-h-[80px] resize-none border-slate-200 text-sm p-3 leading-snug" 
             />
          </div>
        </div>

        {/* Meio: Tabela de Insumos */}
              ...
              <Table>
                <TableHeader className="bg-white border-b sticky top-0 z-10">
                  <TableRow className="hover:bg-transparent">
                    ...
                    <TableHead className="text-xs font-bold text-slate-600 h-10 text-center">Grupo</TableHead>
                    ...
                    fields.map((field, index) => {
                      ...
                      return (
                        <TableRow key={field.id} className="border-b last:border-none hover:bg-slate-50/50 transition-colors">
                          ...
                          <TableCell className="text-sm font-medium text-slate-800">{currentItem?._name}</TableCell>
                          <TableCell className="text-center text-xs text-slate-500 uppercase">{currentItem?._type}</TableCell>
        ...
        <div className="bg-white border border-slate-200 rounded p-4 flex-shrink-0 space-y-3">
           <div className="flex items-center justify-between">
             ...
             {totals.hasItems && <span className="text-[10px] text-blue-500 font-bold uppercase tracking-tight">Cálculo Automático Ativo (Insumos Detectados)</span>}
           </div>
           <div className="grid grid-cols-4 gap-6">
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  />
                  {totals.hasItems && <div className="absolute inset-0 bg-transparent" title="Este campo é calculado automaticamente a partir dos insumos" />}
                </div>
              </div>
              <div className="space-y-1">
