...
export default function RecebimentosPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"ALL" | "RECEIVED" | "OPEN">("ALL");
  ...
  const { data: summary } = trpc.financial.getReceiptSummary.useQuery();
  ...
  const toggleStatus = trpc.financial.statusToggle.useMutation({
    onSuccess: () => {
      ...
      utils.financial.getReceiptSummary.invalidate();
    },
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter(e => {
      ...
      if (statusFilter === 'RECEIVED') return e.status === 'PAID';
      if (statusFilter === 'OPEN') return e.status === 'PENDING' || e.status === 'PARTIALLY_PAID';
      return true;
    });
    ...
    <div className="flex flex-col h-full bg-[#f8fafc] animate-in fade-in duration-500 print:bg-white print:p-0">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-slate-200 shadow-sm z-10 print:hidden">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-slate-700 tracking-tight">Recebimentos</h1>
        ...
        <div className="flex items-center gap-2">
          <Button asChild className="bg-cyan-500 hover:bg-cyan-600 text-white h-8 text-xs font-bold gap-1.5 rounded-sm shadow-sm">
            <Link href="/dashboard/financeiro/faturamentos">
              <LayoutDashboard className="w-3.5 h-3.5" /> Faturamentos
            </Link>
          </Button>
      ...
      {/* Summary Cards */}
      <div className="px-6 py-3 flex gap-3 bg-white border-b border-slate-100 print:hidden">
        <div 
          onClick={filterToday}
          className="group cursor-pointer border border-slate-200 border-l-4 border-l-orange-500 bg-white px-3 py-2 rounded-sm hover:bg-slate-50 transition-all active:scale-[0.98] shadow-sm flex flex-col min-w-[140px]"
        >
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">A receber hoje</p>
        ...
        <div 
          ...
          className="group cursor-pointer border border-slate-200 border-l-4 border-l-blue-500 bg-white px-3 py-2 rounded-sm hover:bg-slate-50 transition-all active:scale-[0.98] shadow-sm flex flex-col min-w-[140px]"
        >
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">A receber em 7 dias</p>
          <p className="text-sm font-black text-slate-700">
        ...
        <div 
          ...
          className="group cursor-pointer border border-slate-200 border-l-4 border-l-cyan-500 bg-white px-3 py-2 rounded-sm hover:bg-slate-50 transition-all active:scale-[0.98] shadow-sm flex flex-col min-w-[140px]"
        >
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">A receber em 30 dias</p>
          <p className="text-sm font-black text-slate-700">
      ...
      <div className="px-6 py-3 bg-[#f1f5f9] border-b border-slate-200 flex flex-wrap items-center gap-4 shadow-inner print:hidden">
        ...
        <div className="ml-auto flex bg-white border border-slate-300 p-0.5 rounded-sm shadow-sm gap-0.5">
          ...
          <button 
            ...
            className={`px-4 py-1 text-[11px] font-bold rounded-[2px] transition-all ${statusFilter === 'RECEIVED' ? 'bg-blue-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
          >
            Recebidos
          </button>
          <button 
            onClick={() => setStatusFilter("OPEN")}
            className={`px-4 py-1 text-[11px] font-bold rounded-[2px] transition-all ${statusFilter === 'OPEN' ? 'bg-blue-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}
          >
            Em Aberto
          </button>
        </div>
          ...
          <Table className="border-collapse border-y">
            <TableHeader className="bg-slate-50/80 sticky top-0 z-20 backdrop-blur-sm border-b">
              <TableRow className="hover:bg-transparent border-slate-200">
                ...
                <TableHead className="py-2.5 px-4 h-auto text-[10px] font-black uppercase text-slate-600 border-r border-slate-200 text-right">A Receber</TableHead>
                <TableHead className="py-2.5 px-4 h-auto text-[10px] font-black uppercase text-slate-600 border-r border-slate-200">Recebido</TableHead>
                ...
                <TableHead className="py-2.5 px-4 h-auto text-[10px] font-black uppercase text-slate-600 border-r border-slate-200 text-center">Recebido</TableHead>
      ...
      <div className="px-6 py-2 bg-white border-t border-slate-200 flex items-center justify-between text-[10px] font-black text-slate-400 uppercase tracking-tighter print:hidden">
        <div className="flex items-center gap-4">
          ...
          <span className="bg-blue-50 text-blue-600 px-2 py-1 border border-blue-100 rounded-sm shadow-sm">Total Recebido: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(filteredEntries.filter(e => e.status === 'PAID').reduce((acc, curr) => acc + curr.amount, 0))}</span>
          <span className="text-slate-300">|</span>
          <span className="bg-orange-50 text-orange-600 px-2 py-1 border border-orange-100 rounded-sm shadow-sm">A Receber: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(filteredEntries.filter(e => e.status !== 'PAID').reduce((acc, curr) => acc + curr.amount, 0))}</span>
        </div>
