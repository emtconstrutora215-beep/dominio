...
interface InsumoCategorySelectProps {
  value?: string | null;
  onChange: (value: string) => void;
  placeholder?: string;
}

export function InsumoCategorySelect({ value, onChange, placeholder = "Selecione..." }: InsumoCategorySelectProps) {
  ...
  const { data: categories, isLoading } = trpc.catalogItemCategory.list.useQuery();
  const createMutation = trpc.catalogItemCategory.create.useMutation({
    onSuccess: (cat) => {
      toast.success("Tipo cadastrado!");
      utils.catalogItemCategory.list.invalidate();
      onChange(cat.name);
        ...
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">
              {isLoading ? "Carregando..." : (value ? categories?.find((c) => c.name === value)?.name || value : placeholder)}
            </span>
              ...
              <CommandGroup heading="Selecione abaixo">
                {categories?.map((cat) => (
                  ...
                  >
                    ...
                    />
                    <span className="font-semibold text-slate-900 uppercase">{cat.name}</span>
                  </CommandItem>
                ))}
              </CommandGroup>
            </CommandList>
      ...
      {showAddPopup && (
        <div className="absolute top-0 left-0 w-full z-[60] bg-white border border-slate-200 rounded-xl shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200">
           ...
           <Input 
             ...
             placeholder="Ex: AREIA, BRITA, CIMENTO..."
