...
export function BaseSelect({ value, onChange, placeholder = "Selecione..." }: BaseSelectProps) {
  ...
  const { data: bases, isLoading } = trpc.catalogItemBase.list.useQuery();
  const createMutation = trpc.catalogItemBase.create.useMutation({
    onSuccess: (base) => {
      toast.success("Base cadastrada!");
      utils.catalogItemBase.list.invalidate();
      onChange(base.name);
  ...
  const handleSaveNew = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!newBase) return;
    createMutation.mutate({ name: newBase });
  };
    ...
    <div className="relative w-full">
        ...
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">
              {isLoading ? "Carregando..." : (value ? bases?.find((b) => b.name === value)?.name || value : placeholder)}
            </span>
            ...
            <CommandList className="max-h-[300px]">
              <CommandEmpty className="p-4 text-center">
                <div className="flex flex-col gap-3">
                  <p className="text-xs text-slate-500 font-medium">Nenhuma base encontrada.</p>
                  <Button 
                    size="sm" 
                    variant="outline" 
              ...
              <CommandGroup heading="Selecione abaixo">
                {bases?.map((base) => (
                  ...
                  >
                    ...
                    />
                    <span className="font-semibold text-slate-900 uppercase">{base.name}</span>
                  </CommandItem>
                ))}
              </CommandGroup>
            </CommandList>
      ...
      {showAddPopup && (
        <div className="absolute top-0 left-0 w-full z-[60] bg-white border border-slate-200 rounded-xl shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200">
           <p className="text-sm font-semibold text-slate-900 mb-4">Digite o nome da nova base</p>
           <Input 
             autoFocus
             value={newBase}
             onChange={(e) => setNewBase(e.target.value)}
             placeholder="Ex: SINAPI, ORSE, PRÓPRIA..."
           ...
           <div className="flex items-center gap-3">
              <Button 
                ...
                disabled={createMutation.isPending || !newBase}
              >
